    ...
    def set_product_specifications(Long id){
        authenticatedAdminSpecification { adminAccount, specificationInstance ->
            ...
            if(params.productSpecifications){
                ...
                flash.message = messageSource.getMessage("successfully.set.specifications", null, LocaleContextHolder.locale)

            }else{
                flash.message = messageSource.getMessage("no.changes.made", null, LocaleContextHolder.locale)
            }
		...
		authenticatedAdmin { adminAccount ->
			if(params.name){
				...
				if(!params.catalogIds){
					flash.message = messageSource.getMessage("assign.one.catalog.continue", null, LocaleContextHolder.locale)
				...
				if(!catalogSelectedIdsArray){
					flash.error = messageSource.getMessage("assign.one.catalog.continue", null, LocaleContextHolder.locale)
				...
				flash.message = messageSource.getMessage("successfully.added.specification", null, LocaleContextHolder.locale)
	...
	def add_option(Long id){
		authenticatedAdminSpecification { adminAccount, specificationInstance ->
        	if(params.name){
                ...
                if(existingOption){
                    flash.optionMessage =  messageSource.getMessage("option.same.name.error", null, LocaleContextHolder.locale)
                }else{
				...
				flash.optionMessage =  messageSource.getMessage("name.cannot.be.blank", null, LocaleContextHolder.locale)
			}
			redirect(action:'edit', id : specificationInstance.id)
		}
	}
	...
	def update_option(){
		authenticatedAdminSpecificationOption { adminAccount, specificationOptionInstance ->
			if(params.name){
				...
				flash.message = messageSource.getMessage("successfully.update", null, LocaleContextHolder.locale)
			}else{
				flash.message = messageSource.getMessage("name.cannot.be.blank", null, LocaleContextHolder.locale)
			}
