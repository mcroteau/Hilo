...
class ImportController {
	...
	def import_data(){
		...
		try{
			...
			flash.message = messageSource.getMessage("specify.file.import.error", null, LocaleContextHolder.locale)
			...
			if(!existingProduct){
				...
				**/
				if(catalogsExist(data.catalogs)){
					...
					flash.message = messageSource.getMessage("catalogs.imported.before.message", null, LocaleContextHolder.locale)
				}	

				count++	
			}else{
				flash.message = messageSource.getMessage("not.all.data.imported.message", null, LocaleContextHolder.locale)
	...
	def saveAdditionalPhotos(additionalPhotos){
		...
		additionalPhotos.each(){ ap ->
			...
			if(!existingAdditionalPhoto){ 
				...
				flash.message =  messageSource.getMessage("catalogs.products.created.message", null, LocaleContextHolder.locale)
			}
		}	
		...
		if(shoppingCarts){
			...
			shoppingCarts.each(){ sc ->
				...
				if(!existingShoppingCart){
					...
					flash.message = messageSource.getMessage("shopping.carts.exist", null, LocaleContextHolder.locale)
				}
			}
		}
