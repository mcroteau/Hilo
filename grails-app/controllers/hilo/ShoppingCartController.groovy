...
import io.hilo.common.ShoppingCartStatus
...
import io.hilo.api.mail.ShippingApiHelper
...
class ShoppingCartController {
    ...
    def index() {	
		if(springSecurityService.isLoggedIn()){
			...
			if(shoppingCartInstance){
				...
				if(!permission){
					flash.message = messageSource.getMessage("shopping.permission.message", null, LocaleContextHolder.locale)
	...
	def add(){
		...
		if(!params.quantity || 
			...
			flash.message = messageSource.getMessage("quantity.number.message", null, LocaleContextHolder.locale)
			redirect(controller : 'product', action : 'details', id : params.id )
			return
		}
		
		if(productInstance.quantity < Integer.parseInt(params.quantity)){
			flash.message = messageSource.getMessage("shopping.cart.quantity.error.message", [ productInstance.quantity ] as Object[], "Default", LocaleContextHolder.locale)
			redirect(controller : 'product', action : 'details', id : params.id )
			return
		}
		...
		**/
		...
		if(!productInstance.purchaseable){
			flash.message = messageSource.getMessage("shopping.cart.purchaseable.message", null, LocaleContextHolder.locale)
		...
		flash.message = messageSource.getMessage("successfully.added.item.cart.message", null, LocaleContextHolder.locale)
	...
	def remove_item(Long id){
		...
		if(shoppingCart && shoppingCartItem){
			flash.message = messageSource.getMessage("successfully.removed.item.cart.message", null, LocaleContextHolder.locale)
			shoppingCartItem.delete(flush:true)
			
			redirect(action:'index', id:id)
		}else{
			flash.message = messageSource.getMessage("Item not found in cart", null, LocaleContextHolder.locale)
			redirect(action:'index', id:id)
		}	
	}
	...
	def checkout_preview(){
		authenticatedPermittedShoppingCart { accountInstance, shoppingCart -> 
			...
			if(!addressComplete(accountInstance)){
				flash.message = messageSource.getMessage("shopping.cart.complete.address", null, LocaleContextHolder.locale)
			...
			if(easypostEnabled == "true"){
				if(!accountInstance.addressVerified){
					...
					flash.message = messageSource.getMessage("verify.address.confirm", null, LocaleContextHolder.locale)
					redirect(controller:'account', action:'customer_profile')
				}
			}
			...
			if(shoppingCart && shoppingCart.status == ShoppingCartStatus.ACTIVE.description()){
				...
				flash.message = messageSource.getMessage("shopping.cart.empty", null, LocaleContextHolder.locale)
	...
	def anonymous_preview(){
		...
		try{
			...
			if(!shoppingCart){
				flash.message = messageSource.getMessage("shopping.cart.not.found", null, LocaleContextHolder.locale)
				redirect(action:'anonymous')
				return
			}
	...
	def checkout(){
		...
		if(springSecurityService.isLoggedIn()){
			...
			if(existingAccount){
				...
				if(!params.name){
					flash.message = messageSource.getMessage("please.enter.valid.name", null, LocaleContextHolder.locale)
					redirect(action:'anonymous')
					return
				}
        		
				if(!params.address1){
					flash.message = messageSource.getMessage("please.enter.valid.address", null, LocaleContextHolder.locale)
					redirect(action:'anonymous')
					return
				}
        		
				if(!params.city){
					flash.message = messageSource.getMessage("please.enter.valid.city", null, LocaleContextHolder.locale)
					redirect(action:'anonymous')
					return
				}
		...
		if(!account.validate()){
			...
			flash.message = messageSource.getMessage("something.went.wrong.no.funds.charged", null, LocaleContextHolder.locale)
		...
		if(shoppingCart){
			...
			if(shoppingCart.status == ShoppingCartStatus.TRANSACTION.description()){
				flash.message = messageSource.getMessage("order.has.been.placed", null, LocaleContextHolder.locale)
				redirect(action:'index')
				return
			}
			...
			try {
				...
				flash.message = messageSource.getMessage("something.went.wrong.information.message", null, LocaleContextHolder.locale)
	...
	def sendOrderToAdmin(transactionInstance){
		...
		try{
			...
			subject = subject +  messageSource.getMessage("order.placed.message", null, LocaleContextHolder.locale)
		...
		if(account){
			...
			if(easypostEnabled == "true" && params.shippingSet != "true"){		
				...
				try{
					...
					shoppingCart.shipping = applicationService.getShipping()
					flash.message = messageSource.getMessage("something.went.wrong.calculating.shipping.message", null, LocaleContextHolder.locale)
	...
	def sendNewOrderEmail(customerAccount, transaction){
		try { 
			...
			def customerSubject = "${applicationService.getStoreName()} : " + messageSource.getMessage("order.confirmation", null, LocaleContextHolder.locale)
