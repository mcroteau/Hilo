...
class CatalogController {
	...
	def products(Long id){
		...
		if(!catalogInstance){
			flash.message = messageSource.getMessage("catalog.not.found", null, LocaleContextHolder.locale)
			redirect(uri: '/',)	
			return
		}
    ...
    def save() {
		authenticatedAdmin { adminAccount ->
			...
			if(params.location){
				...
				if(!parentCatalog){
					flash.message = messageSource.getMessage("catalog.location.error.message", null, LocaleContextHolder.locale)
    	    ...
    	    redirect(action: "edit", id: catalogInstance.id)
    	}
    }
    ...
    def delete(Long id) {
		authenticatedAdminCatalog { adminAccount, catalogInstance ->
    	    try {
    	        ...
    	        flash.message = messageSource.getMessage("catalog.products.error.message", null, LocaleContextHolder.locale)
				e.printStackTrace()
	...
	def menu_view(){
		...
		toplevelCatalogs.each{ catalog ->
			...
			catalogMenuString += "(<a href=\"/${applicationService.getContextName()}/catalog/manage_positions/${catalog.id}\">" + messageSource.getMessage("update.positions", null, LocaleContextHolder.locale) + "</a>"
			catalogMenuString += "&nbsp;|&nbsp;"
			catalogMenuString += "<a href=\"/${applicationService.getContextName()}/catalog/edit/${catalog.id}\">" + messageSource.getMessage("edit", null, LocaleContextHolder.locale) + "</a>)"
	...
	def getAllSubcatalogLists(catalog){
		...
		catalog.subcatalogs.each{ subcatalog ->
			...
			subcatalogsMenu += "(<a href=\"/${applicationService.getContextName()}/catalog/manage_positions/${subcatalog.id}\">" + messageSource.getMessage("update.positions", null, LocaleContextHolder.locale) + "</a>"
			subcatalogsMenu += "&nbsp;|&nbsp;"
			subcatalogsMenu += "<a href=\"/${applicationService.getContextName()}/catalog/edit/${subcatalog.id}\">" + messageSource.getMessage("edit", null, LocaleContextHolder.locale) + "</a>)"
