...
class ConfigurationController {
	...
	**/
	...
	private final String MULTI_CURRENCY_ENABLED = "multi.currency.enabled"
	...
	def save_shipping_settings(){
		...
		**/
		...
		if(easypostEnabled == "true"){
			if(testApiKey == ""){
				flash.error = messageSource.getMessage("easypost.blank.test.message", null, LocaleContextHolder.locale)
				redirect(action : 'shipping_settings')
				return
			}
			if(liveApiKey == ""){
				flash.error = messageSource.getMessage("easypost.blank.live.message", null, LocaleContextHolder.locale)
				redirect(action : 'shipping_settings')
				return
			}
				...
				try{
					...
					**/
					...
					if(!shipmentApi.validAddress(address)){
						flash.error = messageSource.getMessage("address.verified.error.message", null, LocaleContextHolder.locale)
	   					redirect(action : 'shipping_settings')
	   					return
					}
	...
	def upload(){
		authenticatedAdmin{ adminAccount ->
			...
			if(file){
				...
				flash.message = messageSource.getMessage("successfully.uploaded", null, LocaleContextHolder.locale)
				redirect(action:'uploads')
			}else{
	...
	def import_products(){
		authenticatedAdmin{ adminAccount ->
			...
			if(count == 0 && skipped > 0){
				flash.message = messageSource.getMessage("product.exists.error.message", null, LocaleContextHolder.locale)
			}
 	   		if(count > 0){
				flash.message = messageSource.getMessage("successfully.imported.products", [ count ] as Object[], "Defaul", LocaleContextHolder.locale)
			}
			if(errored > 0){
				flash.error = messageSource.getMessage("errored.imported.products", [ errored ] as Object[], "Default Message change it for local", LocaleContextHolder.locale)
			}
	...
	def generate_key(){
		...
		flash.message = messageSource.getMessage("successfully.generated.key", null, LocaleContextHolder.locale)
	...
	@Secured(['ROLE_ADMIN'])
	def enable_multicurrency(){	
		return setMultiCurrencySetting("true")
	}


	@Secured(['ROLE_ADMIN'])
	def disable_multicurrency(){	
		return setMultiCurrencySetting("false")
	}




	def setMultiCurrencySetting(enabled){
		...
		try{
			...
			prop.setProperty(MULTI_CURRENCY_ENABLED, enabled);
			...
			flash.message = messageSource.getMessage("successfully.set.multi.currency", null, LocaleContextHolder.locale)
			redirect(action : 'payment_settings')
