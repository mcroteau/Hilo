...
class ShippingController {
	...
	def set(){
		...
		if(shoppingCart){
			if(params.optionId && 
				...
				flash.message = messageSource.getMessage("shipment.carrier.invalid.message", null, LocaleContextHolder.locale)
	...
	def select(){
		...
		if(shoppingCart?.account){
			...
			flash.message = messageSource.getMessage("shipment.account.information.missing.message", null, LocaleContextHolder.locale)
			redirect(controller: "store", action:"index")
		}
