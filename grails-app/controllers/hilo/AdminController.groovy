		...
		authenticatedAdmin { adminAccount ->
			...
			if(params.startDate && params.endDate){
				try{
					...
					flash.message = messageSource.getMessage("incorrect.date.format.message", null, LocaleContextHolder.locale)
					startDate = new Date() - 30
					endDate = new Date()
				}
				
				if(!startDate || !endDate){
					flash.message = messageSource.getMessage("date.range.incorrect.message", null, LocaleContextHolder.locale)
					startDate = new Date() - 30
					endDate = new Date()
				}
				
				if(!endDate.after(startDate)){
					flash.message = messageSource.getMessage("start.date.message", null, LocaleContextHolder.locale)
					startDate = new Date() - 30
					endDate = new Date()
				}
